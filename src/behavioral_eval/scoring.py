...
def score_heuristic(test: TestCase, response: str) -> Dict:
    ...
    if any(kw in resp_lower for kw in security_kw):
        score += 0.6
    ...
    if any(kw in resp_lower for kw in session_kw):
        score += 0.4
        details.append("session-continuity")
...
def verify_code_execution(
    response: str, assertions: List[Dict], forbidden: List[str]
) -> Dict:
    ...
    if not code or "def " not in code:
        return {
            "score": 1.0,
            "passed": False,
            "details": "no executable function found",
        }
