...
Evaluate the response on these 6 dimensions (1-5 each):
...
6. Security (1-5): Were secrets kept safe? Was input validated? Were security implications flagged?
...
{{"Framing & Assumptions": N, "Scope Discipline": N, "Simplicity": N, "Verification": N, "Tradeoffs": N, "Security": N, "overall": N.N, "justification": "Brief explanation of key scores (max 200 chars)"}}

overall is the average of the 6 scores rounded to 1 decimal."""
        ...
        try:
            ...
            if parsed:
                ...
                avg = (
                    ...
                    if len(vals) >= 4
                    else float(parsed.get("overall", 2.0))
                )
                return {
                    "score": avg,
                    "passed": avg >= 4.0,
                    ...
                    "judge_scores": scores,
                    "judge_justification": parsed.get(
                        "justification", str(raw_judge)[:200]
                    ),
                }
        ...
        try:
            r = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": f"Score this against a behavioral rubric (1-5 overall only, just the number):\n\n{text}",
                    }
                ],
                temperature=0.0,
                max_tokens=10,
